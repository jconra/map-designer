...
// tweak). Re-seeding is guarded by a flag so deleting it keeps it gone. An entry with
...
const RETIRED_PRESETS = ['preset-smugglers-bay'];   // outdated (pre gate/road/rules era)
function purgeRetiredPresets() {
  let changed = false;
  for (const id of RETIRED_PRESETS) {
    const i = savedMaps.findIndex(m => m.id === id);
    if (i < 0) continue;
    try { localStorage.removeItem(MAP_KEY(id)); localStorage.removeItem('mapdesigner:_seeded:' + id); } catch (e) { /* ignore */ }
...
function seedPresets() {
  ...
  for (const pr of PRESET_MAPS) {
    ...
    try {
      ...
      if (pr.file) {
        // Fetched preset (campaign map): seed asynchronously and refresh the list when it lands.
        fetch(pr.file).then(r => r.ok ? r.json() : null).then(cfg => {
          if (!cfg) return;
          try {
            localStorage.setItem(MAP_KEY(pr.id), JSON.stringify(cfg));
            savedMaps.push({ id: pr.id, name: pr.name });
            localStorage.setItem(flag, '1');
            saveMapIndex(); rebuildMapList();
          } catch (e) { /* private/full — skip */ }
        }).catch(() => { /* offline / file missing — try again next boot */ });
        continue;
      }
